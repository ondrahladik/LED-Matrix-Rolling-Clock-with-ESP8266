 ...
 ** Date:    February 2024                                                                **
 ** Version: 9.8.5 (English-UK) - without Holidays                                        ** 
...
// #####################################################################################
  ...
  lc.shutdown(index,false);} 
   ...
   Serial.println("   (Verze: "+LMCVersion+")");
   Serial.println(F(""));
  ...
  if (littleFSInit) {
  LittleFS.format(); 
  printStringWithShift("     +++       Souborovy system byl naformatovan !  \
  --- Nastavte promennou -FormatFileSystem- zpet na -false- a \
  Nahrajte skicu znovu.....",ScrollTextTime);
  printStringWithShift("                              +++\
         Hodiny se restartují      +++           ",ScrollTextTime);
  ...
  if (ssid == "") {
    Serial.println(F(""));
    Serial.println(F("***** Prvni spusteni hodin *****")); // First Start Clock
    Serial.println(F(""));
    }}
  ...
  if (ssid != ""){
  if (SerialMonitor) {  
  Serial.print(F("Pripojeno k WiFi SSID: "));
  Serial.println(ssid);}
  ...
  cip2 = "             IP-Adresa:  "+cip+"                "; // Determined IP address (DHCP)

  printStringWithShift(cip2.c_str(), ScrollTextTime); // Display IP Address  MyWaitLoop(10); // short wait

  if (SerialMonitor) {Serial.println(F("Startuji UDP-Server  ..."));}
  WiFiUDPStart();  // Start UPD Server

  // #######################################################################################
  // Arduino OTA/DNS
  if (SerialMonitor) {Serial.println(F("Startuji Arduino OTA/DNS-Server ... - URL: http://ledmatrixclock.local"));}
  ...
  if (!WeatherFunctions) {
  Serial.println(F(""));
  Serial.println(F("OpenWeatherMap byla deaktivovana – zadna data o pocasi"));
  Serial.println(F("")); 
   } else {Serial.println(F(""));Serial.println(F(""));}

  if (!NewsAPIFunction) {
  Serial.println(F(""));
  Serial.println(F("NewsAPI byla deaktivovana - zadna data o zpravach"));
  Serial.println(F("")); 
   } else {Serial.println(F(""));Serial.println(F(""));}}
    ...
    if (littleFSInit) {  
    if (SerialMonitor) {  
    Serial.println(F("Little Filesystem Init - hotovo."));
    FSInfo fs_info;
    LittleFS.info(fs_info);
    Serial.println(F("File system info."));
    Serial.print(F("Celkovy prostor:      "));
    Serial.print(fs_info.totalBytes);
    Serial.println(F("byte"));
    Serial.print(F("Celkovy pouzity prostor: "));
    Serial.print(fs_info.usedBytes);
    Serial.println(F("byte"));
    Serial.print(F("Velikost bloku:       "));
    Serial.print(fs_info.blockSize);
    Serial.println(F("byte"));
    Serial.print(F("Velikost stranky:        "));
    Serial.print(fs_info.totalBytes);
    Serial.println(F("byte"));
    Serial.print(F("Maximalni pocet otevrenych souboru:   "));
    Serial.println(fs_info.maxOpenFiles);
    Serial.print(F("Maximalni delka cesty:  "));
   ...
   if (SerialMonitor) {  
   Serial.println(F("Little Filesystem Init - Chyba."));}}
  if (SerialMonitor) {   
  ...
  if (DHT11Sensor) { 
  if (SerialMonitor) {Serial.println("DHT11 Senzor je pripojen. ("+InTempValue+"°C)");}} else {
  if (SerialMonitor) {Serial.println("DHT22 Senzor je pripojen. ("+InTempValue+"°C)");}}  
  if (SerialMonitor) {Serial.println(F(""));}} else {
  if ((InTempValue != "!" && InHumValue == "-") || (InTempValue == "!" && InHumValue != "-")) {
  InHumValue = F("!");    
  if (SerialMonitor) {
  Serial.println(F(""));  
  Serial.println(F("Porucha DHT snímače !"));
  Serial.println(F(""));}} else {
  InHumValue = F("!"); 
  if (SerialMonitor) {
  Serial.println(F(""));  
  Serial.println(F("Neni pripojen snimac DHT"));
  Serial.println(F(""));}
  if (SerialMonitor) {Serial.println(F(""));}}}}
  ...
  if (SerialMonitor) {Serial.println("Matrix vypnuty v "+ String(h) + ":" + String(m));}}}

  if (!IsBuzzer) {
  if (SerialMonitor) {Serial.println(F("Buzzer neni aktivni"));}} else {
  if (SerialMonitor) {Serial.println(F("Buzzer je aktivni"));}}
  ...
  if (ssid != ""){ 
  if (SerialMonitor) {Serial.println(F("Pripojeni WiFi se nezdarilo!"));}  
  printStringWithShift("        +++  Pripojeni WiFi se nezdarilo !  +++           ",ScrollTextTime);
  ...
  if (SerialMonitor) {
  Serial.println(F(""));  
  Serial.println(F("Porucha snimace DHT !"));
  Serial.println(F(""));}} else {
  InHumValue = F("!"); 
  if (SerialMonitor) {
  Serial.println(F(""));  
  ...
  if (SerialMonitor) {  
  Serial.println(F("Zapinam AP ... [LED-Matrix-Clock]")); 
  Serial.println(F(""));}
  String APIPAdresse = "        +++       AP je spusteno      +++       IP-Adresa: "+String(APIPA1)+"."+String(APIPA2)+"."+String(APIPA3)+"."+String(APIPA4)+"      +++           ";
...
// contrast(); //AutoBrightness
 ...
 if (!await) {MatrixButtonOnOff();} 
  ...
  if (Display){
  printStringWithShift("        +++       WiFi pripojeni: odpojeno       +++           ",ScrollTextTime); 
  MyWaitLoop(100); // 0,1 sek. wait
  if (SerialMonitor) {Serial.println("Prosim cekejte... "+FormatedDateTimeEN(false));}
  printStringWithShift("        +++       Prosim cekejte...      +++           ",ScrollTextTime);}
  ...
  StartStopServers -= 1;
  if (SerialMonitor) {Serial.println("WiFi pripojeni: obnoveno - "+FormatedDateTimeEN(false));}
  WiFiUDPStart(); // start UDP-Server
  WebServerStart(); // start Webserver
  if (Display) {printStringWithShift("        +++       WiFi pripojeni: obnoveno...       +++           ",ScrollTextTime);}
  MyWaitLoop(1000);
  sendCmdAll(CMD_SHUTDOWN,0); // Display off
  ...
  if (!ClockWiFiConnectFail) { 
  HandleOTAandWebServer();  // OTA and Web Server query 
  if (StartFail) {if (SerialMonitor) {Serial.println(F("Hodiny se restartuji..."));}
...
if (WatchOwner1FirstName != "") {
String SHT= "       +++   Vitejte  "+cleanText(WatchOwner1FirstName)+"    ---    Hodiny jsou spusteny...       +++                    "; 
printStringWithShift(SHT.c_str(), ScrollTextTime);} else {
printStringWithShift("       +++     Hodiny jsou spusteny...     +++                    ", ScrollTextTime);} 
// ***************************************************************************************************************************
...
await = false;} // Start of the clock - End
  ...
  if (SerialMonitor) {Serial.println(F("Zobrazuji Ducha"));}  
  ...
  printStringWithShift(SText.c_str(),ScrollTextTime);
  ClockScrollIn(); // Scroll the Time in the Display
  if (SerialMonitor) {Serial.println("Matrix zapnuty v "+ String(h) + ":" + String(m));} 
  Display = true;
  ...
  printStringWithShift("        +++       Upozorneni: Vlhkost v miste hodin je prílis nizka... !      +++           ",ScrollTextTime);
  MyWaitLoop(2500);}
  ...
  for (int i=0; i <= 1; i++){ // Repeat message 2x
  printStringWithShift("        +++       Upozorneni: Vlhkost v miste hodin je prílis vysoka...!      +++           ",ScrollTextTime);
  MyWaitLoop(2500);}
  if (IsPushover) {PushOverAlert(InHumidityStatus);}  // Send PushOver Message
  ClockScrollIn();}
  ...
  if  (String(m)+":"+String(s) == OnlyTempViewTime[i]+":11") {    
    ...
    InTempString = "           Vnitrni teplota:          "+String(IT) + " " + InTempValue + deg + "C ";} else {
    InTempString = "           Vnitrni teplota:          "+String(IT2) + " " + InTempValue + deg + "C";}} else {
    if (IntInTempValue > 9) {  
    InTempString = "           Vnitrni teplota:          "+String(IT) + " " + InTempValue + deg + "C ";} else {
    InTempString = "           Vnitrni teplota:          "+String(IT) + "  " + InTempValue + deg + "C ";}} 
    ...
    } else {if (SerialMonitor) {Serial.println(F("Senzor DHT je vypnuty nebo poskozeny"));}
    digitalWrite(HWLEDPin, LOW); // LED turn off
  ...
  if  (String(m)+":"+String(s) == OnlyHumViewTime[i]+":11") { 
    ...
    String InHumString = "";
    if (IntInHumValue > 99) {
    InHumString = "           Vnitrni vlhkost:          "+ String(IL2) + " " + InHumValue + " %";} else {
    if (IntInHumValue < 10) {
    InHumString = "           Vnitrni vlhkost:           "+ String(IL) + "  " + InHumValue + " % ";} else {    
    InHumString = "           Vnitrni vlhkost:          "+ String(IL) + " " + InHumValue + " % ";}}
    SIC = true; // ScrollIn Clock
  ...
  if  (String(m)+":"+String(s) == DateNewsViewTime[i]+":11") { 
    await = true;
    CheckInHumidity(); // Indoor Humidity measurement
    if (NewsAPIFunction && AccessNewsAPI) {getNews();} // get actual News
    ...
    if (!GreetingsToday && NewsAPIFunction && AccessNewsAPI) {
    CheckInHumidity(); // Indoor Humidity measurement
    if (APINews != "") {
    SIC = true; // ScrollIn Clock  
