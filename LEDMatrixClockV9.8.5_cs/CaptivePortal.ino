...
if (server.method() == HTTP_POST) {
    ...
    HTMLString += F("<title>Captivní portál</title>");
    HTMLString += F("<style>");
    ...
    HTMLString += F("<h1>Captivní portál</h1>");
    HTMLString += "<h5>("+ProgramName+")</h5>";
    HTMLString += F("<br/>");
    HTMLString += F("<h6>Přístupová data a nastavení byly uloženy<br />Hodiny se restartují</h6>");
    HTMLString += F("</main>");
    HTMLString += F("</body>");
    HTMLString += F("</html>");
    ...
    if (SerialMonitor) {Serial.println(F("Prístup k datum ulozen... "));} 
    printStringWithShift("        +++       Pristupova data a nastaveni byly ulozeny...      +++           ",ScrollTextTime); 
    if (SerialMonitor) {Serial.println(F("Hodiny se restartuji"));} 
    printStringWithShift("        +++       Hodiny se restartuji      +++           ",ScrollTextTime);
    // Reset
    ESP.restart();} else
     { // Access data was not saved
     if (SerialMonitor) {Serial.println(F("Pristupova data nebyla ulozena !"));}
     HTMLString = F("<!doctype html><html lang='en-GB'>");
    HTMLString += F("<head><meta charset='utf-8'>");
    HTMLString += F("<meta name='viewport' content='width=device-width, initial-scale=1'>");
    HTMLString += F("<title>Captivní portál</title>");
    ...
    HTMLString += "<h5>("+ProgramName+")</h5>";
    HTMLString += F("<br/>");
    HTMLString += F("<h6>Chyba při ukládání nastavení<br />Hodiny se restartují</h6>");
    HTMLString += F("</main>");
    HTMLString += F("</body>");
    HTMLString += F("</html>");
    SendHTMLString(HTMLString); // Send HTMLString 
    printStringWithShift("        +++       Pristupova data a nastaveni nebyly ulozeny !     +++           ",ScrollTextTime);
    // Reset
    printStringWithShift("        +++       Hodiny se restartuji      +++           ",ScrollTextTime);
    ESP.restart();}} else {
    // Incomplete settings   
    ...
    HTMLString += "<h5>("+ProgramName+")</h5>";
    HTMLString += F("<br/>");
    HTMLString += F("<h6>Nastavení jsou neúplná nebo neplatná!<br />Nastavení nebyla uložena</h6>");
    HTMLString += F("<br/>");
    HTMLString += F("<a href =\"/\"><button class=\"button\">zpět na domovskou stránku</button></a>");
    HTMLString += F("</main>");
    HTMLString += F("</body>");
    HTMLString += F("</html>");
    SendHTMLString(HTMLString);} // Send HTMLString 
...
int n = WiFi.scanNetworks(false, false); //WiFi.scanNetworks(async, show_hidden)
    ...
    HTMLString += F("<br/>");  
    HTMLString += F("<br/>"); 
    HTMLString += F("<h6>Nebyla nalezena žádná WiFi!</h6>");
    HTMLString += F("<br/>");}
    HTMLString += F("<br/>");
    HTMLString += F("<div class='form-floating'><label>SSID</label><input type='text' class='form-control' name='ssid' id='ssid' value=''></div>");
    HTMLString += F("<div class='form-floating'><br/><label>Heslo</label><input type='password' class='form-control' name='password' id='password' value=''></div>");
    HTMLString += F("<br/>"); 
    // ******************************************* Parameter ************************************************************************************************************************************
    HTMLString += "<div class='form-floating'><br/><label style=\"margin-left: 6px; font-weight: bold;\">+++ OpenWeatherMap a NewsAPI (Volitelnél) +++</label></div>";
    ...
    HTMLString += F("<br/>");
    HTMLString += "<div class='form-floating'><br/><label style=\"margin-left: 15px; font-weight: bold;\">+++ Pushover Messaging Service (Volitelnél) +++</label></div>";
    ...
    HTMLString += "<div class='form-floating'><br/><label>Pushover - Zařízení  1</label><input type='text' maxlength='64' class='form-control' name='podev1' id='podev1' value='"+PushoverDevice1+"'></div>";
    HTMLString += "<div class='form-floating'><br/><label>Pushover - Zařízení  2 (Volitelné)</label><input type='text' maxlength='64' class='form-control' name='podev2' id='podev2' value='"+PushoverDevice2+"'></div>";
    HTMLString += "<div class='form-floating'><br/><label>Pushover - Zařízení 3 (Volitelné)</label><input type='text' maxlength='64' class='form-control' name='podev3' id='podev3' value='"+PushoverDevice3+"'></div>";
    HTMLString += "<div class='form-floating'><br/><label>Pushover - Zvuk zprávy (https://pushover.net/api#sounds)</label><input type='text' maxlength='32' class='form-control' name='poms' id='poms' value='"+PushoverMessageSound+"'></div>";
    HTMLString += F("<br/>");
    if (IsBuzzer) {
    HTMLString += F("<div class='form-floating'><br/><label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Použijte pasivní piezo bzučák&nbsp;");
...
String CPRootHTMLString_3(){
    ...
    HTMLString += F("<button type='submit'>Obnovit</button>");
    HTMLString += F("</form>");
    HTMLString += F("</main>");
    HTMLString += F("</body>");
    HTMLString += F("</html>");
...
HTMLString += "<h5>("+ProgramName+")</h5>";
HTMLString += F("<br/>");
HTMLString += F("<h6>Stránka nebyla nalezena !</h6>");
HTMLString += F("<br/><br/>");
HTMLString += F("<a href =\"/\"><button class=\"button\">zpět na domovskou stránku</button></a>");
HTMLString += F("</main>");
HTMLString += F("</body>");
