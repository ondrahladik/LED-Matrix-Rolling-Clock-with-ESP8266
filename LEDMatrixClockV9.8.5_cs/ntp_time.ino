...
time_t getNtpTime() {
  ...
  if (WaitForNTPIP > 2) {  
  printStringWithShift("        +++        Server NTP neodpovida !      +++           ",ScrollTextTime);
  MyWaitLoop(1000);
  printStringWithShift("        +++        Zkontrolujte pripojeni k internetu      +++           ",ScrollTextTime);
  MyWaitLoop(1000);
  // Neustart Uhr
  ...
  while (millis() - beginWait < 1500) {
    ...
    if (size >= NTP_PACKET_SIZE) {
      WaitForNTPIP = 0;
      if (SerialMonitor) {Serial.println(F("Server NTP neodpovida"));}
  ...
  if (SerialMonitor) {Serial.println("Server NTP neodpovida - "+FormatedDateTimeEN(false));}
  // return 0; // returns 0 if the time cannot be determined.
  printStringWithShift("        +++        Server NTP neodpovida !      +++           ",ScrollTextTime);
  // Clock Restart
  if (SerialMonitor) {Serial.println("Hodiny se restartuji  - "+FormatedDateTimeEN(false));}
  printStringWithShift("        +++       Hodiny se restartuji       +++           ",ScrollTextTime);
  ESP.restart();
  return false;}
